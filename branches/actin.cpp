...
int actin::collisiondetection(void)
{
	...
	// ML Threading now set in motion
	//time_t t;
	//time(&t);
	//cout << "(" << t;
	...
	*/
	/*
	  ...
	  {
	  cerr << "numtodo not match :" << numtodo << " should be" << highestnodecount << endl;
	  for (int i = 0; i < NUM_THREADS; i++)
	  {
	  cerr << "Thread " << i << " start: " << collision_thread_data_array[i].startnode
...
void * actin::collisiondetectiondowork(void* arg, pthread_mutex_t *mutex)
{
    ...
    // cout << "start " << dat->startnode
    //     << ", end "   << dat->endnode << endl;
    vect nodeposvec;
    double distsqr,dist;
    double force;
    vect tomove;
    ...
    vect disp;
    // cout << "looping over nodes " << ((dat->endnode) - (dat->startnode)) << endl;
	...
	// cout << " looping over sgp nodes " << nodes_on_same_gridpoint[dat->threadnum].size() << endl;
	for (vector <nodes*>::iterator sameGPnode=nodes_on_same_gridpoint[dat->threadnum].begin(); 
	     sameGPnode<nodes_on_same_gridpoint[dat->threadnum].end() ; sameGPnode++ )
	{
	    ...
	    //cout << "  looping over near nodes " << recti_near_nodes[dat->threadnum].size() << endl;
...
//					}
    ...
    time(&endt);
    //cout << "collision detection, elapsed time: " << endt-startt << endl;
    return (void*) NULL;
}
